...
function Vehicles(): JSX.Element {
  ...
  useEffect(() => {
    const fetchVeiculoData = async () => {
      if (isEditMode && editId) {
        setFetching(true);
        try {
          const response = await fetch(`${API_BASE_URL}/veiculos/${editId}`);

          if (!response.ok) {
            throw new Error("Erro ao carregar dados do veículo");
          }

          const veiculoData = await response.json();
          setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nome) {
      showError("Por favor, preencha o nome do veículo");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (isEditMode) {
        response = await fetch(`${API_BASE_URL}/veiculos/${editId}`, {
          method: "PUT",
          headers: {
        ...
        response = await fetch(`${API_BASE_URL}/veiculos`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      if (response.ok) {
        ...
        showError(
          errorData.message ||
            `Erro ao ${isEditMode ? "atualizar" : "cadastrar"} veículo`
        );
      }
    } catch (error) {
      console.error(error);
